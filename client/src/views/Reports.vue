    ...
    <div v-else>
          ...
          <div class="bar-chart">
            <div v-for="month in monthlyData" :key="month.month" class="bar-wrapper">
              <div class="bar-container">
                <div
                  class="bar"
                  :style="{ height: (month.revenue / maxRevenue * 100).toFixed(1) + '%' }"
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="stat-value">${{ summaryStats.totalRevenue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">{{ t('reports.avgMonthlyRevenue') }}</div>
          <div class="stat-value">${{ summaryStats.avgMonthlyRevenue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
        </div>
        <div class="stat-card">
...
const summaryStats = computed(() => {
  ...
  const avgMonthlyRevenue = monthlyData.value.length > 0 ? totalRevenue / monthlyData.value.length : 0
  const bestQuarter = quarterlyData.value.reduce(
    (best, q) => (q.total_revenue > (best?.total_revenue ?? 0) ? q : best),
    null
  )?.quarter ?? '-'
  return { totalRevenue, totalOrders, avgMonthlyRevenue, bestQuarter }
})
...
const formatMonth = (monthStr) => {
  ...
  const key = monthKeys[parseInt(parts[1]) - 1]
  return t(`months.${key}`) + ' ' + parts[0]
}
